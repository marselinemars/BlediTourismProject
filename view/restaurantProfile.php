...
<html lang="en">
    <head>
        <title>restaurant profile - bleditourism</title>
...
.guideprofie-container03 {
    ...
    background-image: url("https://images.unsplash.com/photo-1559339352-11d035aa65de?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NDB8fHJlc3RhdXJhbnR8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=500&q=60");
  }
    ...
    <div>
      ...
      <div class="guideprofie-container">
        <div class="guideprofie-container01">
          ...
          <a href="index.php?action=all-restaurants" class="guideprofie-navlink7 button">
...
?>
        ...
        <button id="add1" onclick = close_dialog(event) style="margin:20px auto 10px ; width:100%;" class = "button guideprofie-navlink6 " type="submit" name="submit-btn">Add</button>
          ...
          for ($i=0;$i<count($feedbacks3);$i++){
            $starhtml= generateStarRating ($feedbacks3[$i]['rating']);
            ?>
          ...
          <div class="feedback-details">
            <p class="author"><?php echo htmlspecialchars($feedbacks3[$i]['visitor']);?> </p>
            <p class="comment"><?php echo htmlspecialchars($feedbacks3[$i]['opinion']);?></p>
          </div>
...
<button  onclick="close_dialog(event)" class="close-button">&#10006;</button>
<form id="newfeedback_form" action="index.php?action=restaurant_add_feedback"  method="post">
...
function uploadImage() 
{
  ...
  xhr.open('POST', 'index.php?action=restaurant_add_to_gallery', true);
...
function add_feedback() 
{
  ...
  // Define the AJAX request
  xhr.open('POST', 'index.php?action=restaurant_add_feedback', true);
