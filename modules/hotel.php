...
switch($vars['action']){
    ...
    case "hotel_add_feedback":{
                
        
           
        $info = hotel_add_feedback();
   ...
   case "hotel_profile":{
    ...
    if (isset($_GET['name'])){
      ...
      $items = $db->query('SELECT * FROM hotel WHERE name =? ', $name )->fetchAll();


    }else{


      $items = $db->query('SELECT * FROM hotel WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();

      //getting the guide circuits  
      $name = $items[0]['name'];

    }
        $rooms= $db->query('SELECT title , description , date  FROM rooms WHERE hotel=?' , $name )->fetchAll();

         
         $feedbackshotel= $db->query('SELECT visitor , rating , opinion   FROM feedbackshotel WHERE hotel=?' , $name )->fetchAll();
         
        //getting the guide portfolio 
        $images= hotel_get_gallery ($name)[0];
        $pfp= hotel_get_gallery ($name )[1];
        $info=hotel_get_info($name);
        

        include("view/hotelProfile.php");
        exit;
    ...
    case "hotel_add_to_gallery":{

        $targetimage = hotel_add_image_to_gallery();
        $info = hotel_add_info_to_gallery();
    ...
    case "all-hotels":{
        ...
        $city='Algeria';
        $items = $db->query('SELECT name , address , contact ,  gallery_path , num_of_posts , city FROM hotel ' )->fetchAll();
