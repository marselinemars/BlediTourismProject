...
function agency_get_gallery ($name){

    global $db;
    $items = $db->query('SELECT gallery_path FROM agency WHERE name = ? ',$name )->fetchAll();


    $directory = "agency_galleries/".$items[0]['gallery_path']."/"; // The directory where your images are stored
...
function agency_add_image_to_gallery (){



    global $db;
    $items = $db->query('SELECT gallery_path , num_of_posts FROM agency WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();
...
function agency_add_info_to_gallery (){
...
function agency_add_circuit (){


  global $db;
  $items = $db->query('SELECT name  FROM agency WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();
      ...
      $db->query('INSERT INTO circuits2 (agency, title , description,date) VALUES ( ? , ? , ? , ? ) ' , $name , $title , $description , $date ); 
...
function agency_add_feedback (){
  ...
  $items = $db->query('SELECT name FROM visitor WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();
      ...
      $db->query('INSERT INTO feedbacks2 (visitor , rating , opinion , agency) VALUES ( ? , ? , ? , ? ) ' , $v_name , $rating , $opinion,$name ); 
