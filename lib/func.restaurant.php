...
function restaurant_get_gallery ($name){

    global $db;
    $items = $db->query('SELECT gallery_path FROM resto WHERE name = ? ' , $name  )->fetchAll();


    $directory = "resto_galleries/".$items[0]['gallery_path']."/"; // The directory where your images are stored
...
function restaurant_add_image_to_gallery (){



    global $db;
    $items = $db->query('SELECT gallery_path , num_of_posts FROM resto WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();
...
function restaurant_add_info_to_gallery (){
...
function restaurant_get_info ($name){
...
function restaurant_add_feedback (){


  $_COOKIE['restaurant-name']='sfdgmsldfkg';
  ...
  $items = $db->query('SELECT name FROM visitor WHERE LOWER(email) = ? AND pass = ? ' , $_COOKIE['app_email'] , $_COOKIE['app_pass'] )->fetchAll();
      ...
      $db->query('INSERT INTO feedbacks3 (visitor , rating , opinion , restaurant) VALUES ( ? , ? , ? , ? ) ' , $v_name , $rating , $opinion, $name  ); 
